...
def sex(population, scores):
    """Takes top two population members, makes two children, and picks the fittest between children and parents"""
    result = []
    scored = zip(scores, population)
    scored = sorted(scored)
    mama = scored[0][1]
    papa = scored[1][1]
    result.append(mama); result.append(papa)
    length = len(mama)
    index = int(length/2)
    baby=mama[:index] + papa[index:]
    result.append(baby)
    baby2 = papa[:index] + mama[index:]
    result.append(baby2)
    return result

def evolve(target, num=1000,  mutation_rate=0.01, generation=0):
    ...
    while 1:
        ...
        sexedPop = sex(population, scores)
        scores = [score(seq, target) for seq in sexedPop]
        best_score, best_seq = select(sexedPop, scores)

        #Changed to py3 print function
        print (generation, '\t', best_score, '\t', ''.join(best_seq))
        if best_score == 0: break
        population = breed(best_seq, num, mutation_rate)
        generation += 1
if __name__=='__main__':
    evolve('its the remix to ignition hot and fresh out the kitchen')
